<?php $background = trim((string) ($data['background_image'] ?? '')); $variant = preg_replace('/[^a-z0-9_-]/i', '', (string) ($data['variant'] ?? '')); ?>
<section class="page-banner customizer-banner<?= $variant !== '' ? ' customizer-banner--'.esc($variant) : '' ?>" aria-labelledby="customizer-banner-title"<?= $background !== '' ? ' style="background-image:url(\''.esc($background).'\')"' : '' ?>>
